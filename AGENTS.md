...
- One wallet interval is indexed/transformed per run, while the complete live artifact retains every successfully published wallet; the Gitcoin demo target belongs only to the checked-in historical snapshot and is never a live API fallback.
...
- Static JSON is only the bounded snapshot-backed GitHub Pages demo path. Its checked-in mainnet account evidence is pinned to the recorded safe block. A separate synthetic fixture with an empty typed account-evidence relation exists only for deterministic edge-case tests.
...
- Token symbols/names, registry recognition, ENS resolution, and account type are sourced, time-varying enrichment—not identity facts. Live ENS resolution records its resolver source and finalized observation time/block/hash; the demo's Gitcoin label is official external attribution for a pinned address, not ENS evidence.
