...
The first successful build records one wallet-scoped `ops.pipeline_runs` row from HyperIndex `_meta.startBlock` through the chosen finalized block for the selected wallet. Set `EVM_WALLET_SCAN_ADDRESS` to a configured address when `wallets.csv` contains more than one wallet; the value is normalized before selection, and an unset selector with multiple wallets fails clearly. Each later snapshot for the selected wallet begins at that wallet's previous completed `to_block + 1`; failed rows do not advance coverage and the same interval remains retryable. A run records its finalized end-block hash and completes only after dbt succeeds. The live DuckDB artifact is rebuilt for the selected wallet, and separate wallet runs are not merged into one artifact. Adding wallet B therefore creates no run for wallet A; switching from A to B does not preserve A's materialized analytics or combine their coverage metadata. Treat multi-wallet artifact merging as an unimplemented architecture decision, not as complete combined coverage. `HYPERINDEX_GRAPHQL_URL` or `analytics.hyperindex_graphql_url` may override the local GraphQL default.
