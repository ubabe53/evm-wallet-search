...
The first successful build records one wallet-scoped `ops.pipeline_runs` row from HyperIndex `_meta.startBlock` through the chosen finalized block for each new wallet. Each later snapshot for an existing wallet begins at that wallet's previous completed `to_block + 1`; failed rows do not advance coverage and the same interval remains retryable. A run records its finalized end-block hash and completes only after dbt succeeds. Adding a wallet creates an independent first-scan run and does not alter or rerun existing wallets. The dbt source still rebuilds complete configured wallet history through the finalized end block, preserving prior rows in the same live DuckDB. If no newer finalized block exists for a wallet, that wallet gets no fictitious run while other pending wallets may proceed. `HYPERINDEX_GRAPHQL_URL` or `analytics.hyperindex_graphql_url` may override the local GraphQL default.
