...
- `timeline`: one complete, gap-filled UTC series with exact total, inbound, and outbound captured-event counts. `interval=year` returns the stable observed-year domain; `interval=month` requires one `year` and returns only that calendar window through the latest observed month when it is the boundary year. Recognition, account, and search filters can zero a bucket but do not remove its calendar position;
