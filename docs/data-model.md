...
- `timeline`: one complete, gap-filled UTC bucket series at `day`, `week`, or `month` interval, with exact total, inbound, and outbound captured-event counts;
...
The React live adapter sends the same recognition, account, and search predicates to every endpoint, displays summary counts from the complete matching set, and distinguishes those totals from bounded token, counterparty, and event rows. It requests the timeline without the currently selected date range so all buckets remain available for navigation. Selecting a bucket then sends its exact half-open UTC `start`/`end` pair to summary, event, token, and counterparty requests. Event expansion follows `next_cursor`; it never infers completeness from the current browser array. The static adapter remains separate, reads only generated fixture JSON, and does not claim that its bounded rows support complete period cross-filtering.
