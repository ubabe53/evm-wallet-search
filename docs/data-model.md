...
The checked-in public-demo fixture contains exactly 100 synthetic example events spanning five UTC calendar years and at least 40 months. Its composition deliberately exercises inbound, outbound, self, direct, confirmed-indirect, and legacy-unknown sender evidence; five emitting contracts; both recognition states; repeated and distinct counterparties; monthly timeline navigation; and the dashboard's bounded event disclosure. Fixture dates, block numbers, hashes, addresses, and participant pairs are deterministic examples, not assertions that these combinations occurred on Ethereum. The fixed `0xeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeee` address and `Example wallet` label provide synthetic presentation context only. No fixture row is a claim about a real wallet's live or historical activity, and the fixture does not establish HyperIndex coverage. `scripts/generate_fixture_events.py` deterministically renders the checked-in CSV, while `tests/test_fixture_seed.py` guards reproducibility and scenario breadth.
