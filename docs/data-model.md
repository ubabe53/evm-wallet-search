...
`value_raw_sum` uses DuckDB `BIGNUM` so the analytical mart preserves integer precision beyond JavaScript's safe-number range. `token_decimals` remains sourced metadata at the same token-contract grain, but no floating-point normalized amount is materialized. Neither field is included in the current API or fixture JSON projection because the dashboard does not display token quantities.
