...
4. dbt joins offline token and observed-at counterparty account-evidence snapshots, separates metadata availability, token quality, and spam reputation, calculates interaction-legitimacy evidence, then builds marts into a source-isolated artifact: `analytics/artifacts/live.duckdb` for HyperIndex or `analytics/artifacts/fixture.duckdb` for deterministic tests and demo generation.
