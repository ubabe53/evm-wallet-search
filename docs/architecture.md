...
- The top 500 token summary rows per status, plus the union of address candidates needed to calculate the exact top 50 counterparties for every one of the 945 non-empty token-status/account-filter selections.
- The 5,000 newest daily timeline rows, written back in chronological order.

Combined status-balanced rows are sorted globally after selection, preserving activity and recency ordering. Counterparty selection explicitly evaluates the cross-product of 15 token-status combinations and 63 inclusive account-filter combinations; Safe and ERC-4337 predicates include independent flags and therefore retain overlaps. Candidates are ranked by their combined transfer-event count before limiting, then every status row for each selected address is exported so browser-side filtering can reconstruct the same ranking. `meta.json` records `15`, `63`, `945`, the candidate-address count, and the exact-ranking guarantee. These bounds affect only `public/data/*.json`. dbt models in `analytics/wallet_analytics.duckdb` retain the complete indexed dataset and remain the source for full-history analysis.
