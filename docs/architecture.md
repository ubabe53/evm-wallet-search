...
In live mode, React sends spam, binary account-evidence, and literal text-search predicates to the API. They are applied first, before summary calculations, ranking, graph limiting, or event pagination. The only account choices are `EOA` and `Contract`; selecting both means all rows, including unresolved internal failures, while selecting one excludes the other and unresolved rows. The dashboard renders exact matching totals separately from the bounded rows returned to each panel and follows the event endpoint's opaque cursor on demand. The fixture demo applies equivalent controls only to its small generated payload. The dashboard hides suspected and reviewed spam by default and exposes one `Include spam` toggle. It does not expose token quality, EIP-7702, unresolved status, or the internal trusted/unverified distinction as filters or badges.
...
The pipeline keeps recognition separate from metadata quality and the existing internal reputation evidence. Automatic `recognition_status` is `recognized` for exact-address membership in at least one selected registry and `other` when no match exists. Reviewed seed overrides may explicitly select either result. `recognition_reason`, `recognition_source`, and `token-recognition-v1` preserve why the automatic result was assigned. `metadata_availability` reports whether name, symbol, and decimals are `complete`, `partial`, or `unavailable`; it says nothing about recognition or trust. The lower-level `token_quality` evidence remains available for internal analysis: `high_confidence` for a reviewed manual approval or at least two registry matches, `listed` for one registry, and `unknown` for RPC-only or absent registry evidence.
