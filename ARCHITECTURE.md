...
The indexer and live analytics are explicit operations. A live build chooses the newest block that is both within HyperIndex's transactional progress and no newer than Ethereum's `finalized` head, records one attempted wallet-scoped interval in `ops.pipeline_runs` and `ops.scan_generations` for each wallet that needs coverage, and advances coverage only after dbt succeeds. A new wallet starts at the configured start block independently; an existing wallet starts at its own latest completed block plus one. Builds must not silently start indexing, backfills, registry refreshes, or enrichment jobs.
