...
- Activity timeline: shows captured Transfer-signature event counts in stacked inbound/outbound UTC bars. Users choose Day, Week, or Month (the default). In live mode, selecting a bar reloads summary cards, ranked counterparties, token flow, and recent events for that exact inclusive-start/exclusive-end period while keeping the whole filtered timeline available; selecting it again or using `Clear period` removes the range. Fixture mode renders its bounded timeline but disables period cross-filtering.
- Etherscan navigation: token symbols open their contract page, visible addresses open their address page, and transaction icons open their transaction page in a new tab.
...
- Zero-address handling: excludes the Ethereum zero address from the counterparty ranking because it represents mint/burn mechanics, while retaining those transfers in timeline, event, and token-flow analytics.
