...
Any exact-address registry match produces the automatic `recognized` classification; no match produces `other`. Registry membership is time-varying evidence, not a security guarantee. The lower-level quality and interaction classifier remain internal, versioned evidence. No price, market-cap, volume, or liquidity API is used. Exact definitions and limitations are documented in `docs/architecture.md` and `docs/data-model.md`.
