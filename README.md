...
- Data provenance: the top-left `Data snapshot` line shows the cumulative `Blocks X–Y · Finalized` range backed by completed runs. Fixture mode says `Coverage not recorded`. The adjacent provenance badge identifies fixture versus HyperIndex data and shows the complete snapshot transfer count. Live summary cards are exact for the active selection; panel labels distinguish returned top-N/page rows from complete matching counts.
