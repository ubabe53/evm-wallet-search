...
- Configured project label: `vitalik.eth` (pinned, not resolved live)
...
`tokens:refresh` (also available as `labels:sync`) refreshes the checked-in exact-address registry from Trust Wallet, Uniswap, CoinGecko, and qualifying Coinbase Exchange Ethereum assets. `labels:enrich` reads self-declared token metadata for the highest-impact `other` contracts. `addresses:enrich` batches pinned-block `eth_getCode` observations for every distinct nonzero, nonself event counterparty and checkpoints them in the ignored local evidence database. Ordinary dbt builds never invoke registry or RPC enrichment, and fixture mode contains no invented account evidence.
...
- `meta.json`: the versioned `dashboard-export-v1` provenance contract, observed fixture block/time extrema, complete source-relation counts, JSON export counts, limits, and sampling state. Fixture scan coverage remains unrecorded.

The demo contract is typed in `src/data.ts` and generated by `scripts/export_dashboard.py` from deterministic fixture marts. Event rows use the same lean delivery projection as the local API: canonical event/order identifiers, token display identity, recognition, direction, nullable indirect evidence, and the account evidence rendered by the dashboard. Token summaries include confirmed indirect inbound and outbound counts. Complete emitted participants, exact raw values, transaction-envelope fields, and detailed provenance remain in the materialized intermediate relation.
...
The fixture exporter bounds rows independently for the nine recognition/address-evidence selections the static dashboard can actually request. The local dashboard computes only the requested selection through DuckDB-backed API endpoints.
...
- Data provenance: a shared `Current selection` strip distinguishes fixture from HyperIndex data and shows cumulative `Blocks X–Y · Finalized` coverage backed by completed runs, generation time, and classified versus eligible nonzero/nonself counterparties. Observed event block/time extrema remain separate from those scan bounds. Fixture mode has observed fixture extrema but still says `Coverage not recorded`. The address-evidence hover text separates address-level from event-weighted coverage and discloses failed and not-checked counts. Summary cards stay minimal and are exact for the active live selection; panel labels distinguish returned top-N/page rows from complete matching counts.
...
`token_rpc_metadata.csv` stores pinned-block `name`, `symbol`, and `decimals` responses for `other` contracts. Run `bun run labels:enrich --limit 100` to process the next unenriched contracts by transfer count, `--retry-failed` to retry failures, or `--refresh` to reread already attempted contracts. RPC metadata never establishes recognition, and self-declared names and symbols remain display metadata rather than classification signals.
...
Any exact-address registry match produces the automatic `recognized` classification; no match produces `other`. Registry membership is time-varying evidence, not a security guarantee. No price, market-cap, volume, or liquidity API is used. Exact definitions and limitations are documented in `docs/architecture.md` and `docs/data-model.md`.
