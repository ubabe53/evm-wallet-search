...
- Filter bar: defaults to `All`, offers `Recognized` and `Other`, and applies recognition plus address-type evidence before filtering events, token summaries, graph elements, counterparties, pagination, and summary cards by token, address, direction, or transaction text. Counterparty and graph ranking use recognition inclusively: an address with both recognition states remains eligible under either relevant selection and keeps its complete in-scope activity count. Hovering or focusing the adjacent information control defines recognition and its limits. Live mode evaluates the selection over complete DuckDB rows; fixture mode evaluates only its bounded demo payload.
- Address type: a two-option multi-select filters every view by user-facing `EOA` or `Contract`. Selecting both means all rows, including unresolved internal RPC failures; `unknown` and EIP-7702 are not separate user controls. Its adjacent hover/focus information control explains that this is pinned-block bytecode evidence rather than permanent identity.
- Data provenance: the top-left `Data snapshot` line shows the cumulative `Blocks X–Y · Finalized` range backed by completed runs. Fixture mode says `Coverage not recorded`. The adjacent provenance badge identifies fixture versus HyperIndex data, shows the complete snapshot transfer count, and reports classified versus eligible nonzero/nonself counterparties. Its hover text separates address-level from event-weighted account coverage and discloses failed and not-checked counts. Live summary cards are exact for the active selection; panel labels distinguish returned top-N/page rows from complete matching counts.
- Graph density: shows the 25 highest-ranked counterparties by default, with controls for 10, 25, 50, or 100 direct wallet-address links. It uses the identical ordering as the counterparty table for the same selection. Nodes without a displayed edge are removed automatically.
...
- Graph interaction styling: uses a concentric ego network with the tracked wallet at the center, one counterparty node per address, and one straight aggregate edge per relationship. EOA nodes are circles, Contract nodes are rounded rectangles, and unresolved evidence uses a solid neutral `Unclassified` marker. Hovering a node emphasizes its immediate neighborhood; hovering a link emphasizes that relationship.
- Graph edge labels: show the complete number of captured Transfer-signature events for the wallet-counterparty pair, such as `12 transfers`. They are not labeled as transactions because multiple events can occur in one transaction.
- Etherscan navigation: token symbols open their contract page, visible addresses open their address page, and transaction icons open their transaction page in a new tab. In the graph, clicking a wallet/counterparty node opens its address and clicking an aggregate edge opens Etherscan Advanced Filter with both addresses in both directions and the ERC-20 view selected.
- Address-type badges: graph labels, ranked counterparties, and recent events show only `EOA` or `Contract`. EOA means no ordinary contract bytecode was observed at the pinned block; the tooltip discloses exact EIP-7702 delegation evidence when present and never claims personhood or permanence. Unresolved failures receive no public type badge.
...
- Graph theater mode: expands the counterparty graph over the dashboard for focused exploration, refits Cytoscape to the larger viewport, locks background scrolling, and exits through the collapse control or Escape.
- Token flow: appears below recent events at one row per token. It shows total transfers, a separate self count when present, distinct non-zero/non-self `Senders | Recipients`, and the distinct union of those external counterparties. Sender and recipient mean addresses in emitted token events, not proven people or transaction initiators. Exact raw values remain strings and token-decimals metadata remains available separately, but normalized quantities are intentionally deferred until an exact amount contract has a trustworthy use.
- Direction semantics: recent events are `in`, `out`, or `self`, derived only from emitted `Transfer.from` and `Transfer.to` relative to the tracked wallet. A self-transfer has the wallet on both sides, remains one event, and is excluded from inbound/outbound counts, counterparty totals, rankings, and the graph.
...
Any exact-address registry match produces the automatic `recognized` classification; no match produces `other`. Registry membership is time-varying evidence, not a security guarantee. The lower-level quality and spam classifiers remain internal, versioned evidence. No price, market-cap, volume, or liquidity API is used. Exact definitions and limitations are documented in `docs/architecture.md` and `docs/data-model.md`.
