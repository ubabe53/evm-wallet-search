...
- Graph interaction styling: uses a terminal-inspired network canvas with thin weighted links. Token symbols appear on links instead of occupying separate graph nodes, and resolved counterparties are labeled only as EOA or Contract. Hovering a node emphasizes its immediate neighborhood; hovering a link emphasizes that interaction.
