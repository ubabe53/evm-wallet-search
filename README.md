...
- Dashboard scope: the header calls the product `Transfer Event Analytics`, keeps the compact `Transfers` KPI, and states that the view is based on emitted `Transfer(address,address,uint256)` events. This source signature is shared with ERC-721 and does not prove ERC-20 compliance.
