...
Report only concrete correctness, security, data-integrity, regression, portability, or materially missing-test problems. Do not block on formatting, personal style, speculative improvements, or pre-existing problems outside the staged change. A finding must identify a specific failure mode and the smallest relevant staged file location.
