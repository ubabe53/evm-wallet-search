  ...
  select
    *,
    least(100,
      ...
      + case when is_broad_spray and appears_wallet_outbound then 20 else 0 end
    ...
    concat_ws('; ',
      ...
      case when is_broad_spray and appears_wallet_outbound then 'mass_outbound_without_initiator_proof' end
    ) as interaction_legitimacy_reasons
  from signal_flags
)

select
  * exclude (is_broad_spray, is_one_way, appears_wallet_outbound, interaction_legitimacy_reasons),
  case
  ...
  'interaction-legitimacy-v1' as interaction_legitimacy_version
from scored
